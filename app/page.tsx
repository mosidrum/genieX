...
export default function DashboardPage() {
  ...
  useEffect(() => {
    const freelancerMap = new Map<string, Freelancer>(mockFreelancers.map((f) => [f.id, f]));
  ...
  const selectedWorklog = selectedWorklogId
    ? worklogs.find((w) => w.id === selectedWorklogId)
    : null;
  const selectedFreelancer = selectedWorklog ? freelancers.get(selectedWorklog.freelancerId) : null;
  const selectedTimeEntries = selectedWorklogId
    ? (timeEntriesMap.get(selectedWorklogId) ?? [])
    : [];
